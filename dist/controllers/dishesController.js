...
// ========================
async function getAllDishes(_req, res) {
    try {
...
async function createDish(req, res) {
    try {
        ...
        if (req.file) {
            finalImageUrl = await (0, supabaseStorage_1.uploadImageToSupabase)(req.file);
        }
...
async function updateDish(req, res) {
    try {
        const { id } = req.params;
        const { name, description, price, category, available, imageUrl } = req.body;
        const existing = await prisma_1.prisma.dish.findUnique({ where: { id: Number(id) } });
        if (!existing)
        ...
        if (req.file) {
            ...
            if (existing.imageUrl) {
                await (0, supabaseStorage_1.deleteImageFromSupabase)(existing.imageUrl);
            }
        }
        else if (imageUrl !== undefined) {
            // Sinon, utiliser l'URL fournie en body si elle existe
            const newImageUrl = typeof imageUrl === "string" && imageUrl.trim().length > 0 ? imageUrl.trim() : null;
...
async function deleteDish(req, res) {
    try {
        ...
        await prisma_1.prisma.dish.delete({ where: { id: Number(id) } });
